...
function TimelineCard({ item, index, type = "edu" }: { item: (typeof education)[0]; index: number; type?: "edu" | "achieve" }) {
  ...
  return (
    ...
    >
      <div className={`absolute ${isAchievement ? "right-[-35px]" : "left-[-35px]"} top-8 w-3 h-3 rounded-full bg-[#0b0b0b] border-2 border-[#ff7a00] ring-4 ring-[#0b0b0b]`} />
      
      <div className="flex items-start justify-between gap-4 mb-3">
        <p className="text-[#ff7a00] font-mono text-xs font-bold tracking-tighter">{item.year}</p>
        <span className="text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-md bg-[#ff7a00]/10 text-[#ff7a00] border border-[#ff7a00]">
          {item.badge}
        </span>
      </div>
      
      <h4 className="text-white font-black text-xl leading-tight mb-2 tracking-tight">{item.title}</h4>
      <p className="text-[#a0a0a0] text-sm font-medium leading-relaxed">{item.subtitle}</p>
      {item.institution && (
        <p className="text-[#777777] text-xs mt-3 font-mono">{item.institution}</p>
      )}
    </motion.div>
  );
}
...
export default function EducationCerts() {
  return (
    <section id="education" className="bg-[#0b0b0b] py-24 px-6">
